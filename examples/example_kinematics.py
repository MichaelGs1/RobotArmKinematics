...
def main():
    ...
    def test_ik_consistency_with_fk(q):
        ...
        T06 = fk(q, config.parameter_d, config.parameter_r, config.parameter_alpha, config.parameter_theta, config.parameter_tcp)
        ...
        for _ in range(n_tests):
            ...
            res, q_target = ik(T_test, q, config.parameter_d, config.parameter_r, config.parameter_alpha, config.parameter_theta, config.parameter_tcp, config.parameter_qmin, config.parameter_qmax)

            # 5. Vérification que ik a trouvé une solution
            assert res, "ik n'a pas trouvé de solution"

            # 6. Calculer T_target = fk(q_target, ...)
            T_target = fk(q_target, config.parameter_d, config.parameter_r, config.parameter_alpha, config.parameter_theta, config.parameter_tcp)

            # 7. Vérifier que T_target est proche de T_test
            np.testing.assert_allclose(T_target[:3, 3], T_test[:3, 3], atol=1e-3, err_msg="La position calculée par fk(ik(T_test)) ne correspond pas à T_test")
            np.testing.assert_allclose(T_target[:3, :3], T_test[:3, :3], atol=1e-2, err_msg="L'orientation calculée par fk(ik(T_test)) ne correspond pas à T_test")


    test_ik_consistency_with_fk(q)


    torques = get_torque_gravity(q, config.parameter_d, config.parameter_r, config.parameter_alpha, config.parameter_theta, config.parameter_tcp, config.parameter_masses, config.parameter_cog)
    ...
    #test jacobian
    J = get_jacobian(q, config.parameter_d, config.parameter_r, config.parameter_alpha, config.parameter_theta, config.parameter_tcp)
    print("Jacobian : ", J)
    ...
    for i in range(6):
        ...
        T0 = fk(q, config.parameter_d, config.parameter_r, config.parameter_alpha, config.parameter_theta, config.parameter_tcp)
        T1 = fk(q + dq, config.parameter_d, config.parameter_r, config.parameter_alpha, config.parameter_theta, config.parameter_tcp)
    ...
    pose = np.array([[-0.95035382 , 0.19074547 , 0.2458531,  -0.58782699],
 ...
 [ 0.    ,      0.     ,     0. ,         1.        ]]
    ...
    t1 = perf_counter()
    res, q_target = ik(pose, q, config.parameter_d, config.parameter_r, config.parameter_alpha, config.parameter_theta, config.parameter_tcp, config.parameter_qmin, config.parameter_qmax)
    t2 = perf_counter()
    print("Solution : ", res)
    print("Time : ", t2-t1)
    ...
    torques = get_torque_gravity(q, config.parameter_d, config.parameter_r, config.parameter_alpha, config.parameter_theta, config.parameter_tcp, config.parameter_masses, config.parameter_cog)
    print("Torques : ", torques)
    force = compute_force(q, config.parameter_d, config.parameter_r, config.parameter_alpha, config.parameter_theta, config.parameter_tcp, torques)
